...
type alias AccountTrend = { label : String -- account name
                          , data : List Float }

type alias Model = { labels : List Int -- months
...
asData accounts =
  ...
  , datasets = Array.toList <| map (\{ name, initialValue, incomeEvents } ->
               AccountTrend name (expandAccount 120 0 initialValue incomeEvents))
               accounts }

showData : Model -> Html msg
showData { labels, datasets } =
  ul [] (List.map (\{ label, data }-> li [] [ text label
        , ul [] (List.map (\y -> li [] [ text <| "value: " ++ (toString y) ]) data) ]) datasets)
